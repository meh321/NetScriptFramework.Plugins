...
namespace BlinkSpell
{
    public sealed class Settings
    {
        ...
        public SpellItem SpellForm
        {
            get
            {
                var sp = this._spell;
                if(sp == null && this.SpellFormId != 0 && this.SpellFormFile != null)
                {
                    sp = TESForm.LookupFormFromFile(this.SpellFormId, this.SpellFormFile) as SpellItem;
                    this._spell = sp;
                }
                return sp;
            }
        ...
        public TESForm SoundForm
        {
            get
            {
                var so = this._sound;
                if (so == null && this.SoundFormId != 0 && this.SoundFormFile != null)
                {
                    so = TESForm.LookupFormFromFile(this.SoundFormId, this.SoundFormFile);
                    if (so == null || so.FormType == FormTypes.SoundDescriptor)
                        this._sound = so;
                    else
                        so = null;
                }
            ...
            set
            {
                if (value == null || value.FormType == FormTypes.SoundDescriptor)
                    this._sound = value;
            }
        }
        private TESForm _sound = null;
        ...
        public TESForm IModForm
        {
            get
            {
                var so = this._imod;
                if (so == null && this.IModFormId != 0 && this.IModFormFile != null)
                {
                    so = TESForm.LookupFormFromFile(this.IModFormId, this.IModFormFile);
                    if (so == null || so.FormType == FormTypes.ImageSpaceMod)
                        this._imod = so;
                    else
                        so = null;
                }
            ...
            set
            {
                if (value == null || value.FormType == FormTypes.ImageSpaceMod)
                    this._imod = value;
            }
        }
        private TESForm _imod = null;
        ...
        [ConfigValue("RecoveryTime", "Recovery time", "Set recovery time (cooldown) in seconds.")]
        ...
        [ConfigValue("MarkerFadeInTime", "Marker fade in time", "How many seconds does it take for marker to appear.")]
        public float MarkerFadeInTime
        {
            get;
            set;
        } = 0.1f;

        [ConfigValue("MarkerFadeOutTime", "Marker fade out time", "How many seconds does it take for marker to disappear.")]
        public float MarkerFadeOutTime
        {
            get;
            set;
        } = 0.3f;
